...
class MyEncoderLayer(Module):
    ...
    def forward(
        ...
        if self.norm_first:
            norm_x = self.norm1(x)
            print(norm_x.dtype)
            x = x + self._sa_block(norm_x, src_mask, src_key_padding_mask, is_causal=is_causal)
            print(x.dtype)
            norm_x = self.norm2(x)
            print(norm_x.dtype)
            ...
            attn_x = self._sa_block(x, src_mask, src_key_padding_mask, is_causal=is_causal)
            print(attn_x.dtype)
            x = self.norm1(x + attn_x)
            print(x.dtype)
            ff_x = self._ff_block(x)
            print(ff_x.dtype)
            x = self.norm2(x + ff_x)
            print(x.dtype)
        print()
