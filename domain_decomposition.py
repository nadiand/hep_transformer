...
def transform_trackml_data(event_id, event_dir="/Users/zefwolffs/Documents/phd/tracking/trackformers/data/trackml/train_sample",  overlap_theta=0.1, overlap_phi=0.1, num_bins_theta=5, num_bins_phi=5):
    hits_data = pd.read_csv(f'{event_dir}/event0000{event_id}-hits.csv')
    particles_data = pd.read_csv(f'{event_dir}/event0000{event_id}-particles.csv')
    truth_data = pd.read_csv(f'{event_dir}/event0000{event_id}-truth.csv')
    ...
    data_subdivided, data = split_event(final_data, int(event_id), overlap_theta=overlap_theta, overlap_phi=overlap_phi, num_bins_theta=num_bins_theta, num_bins_phi=num_bins_phi)
...
if __name__ == "__main__":
    ...
    def evaluate_split_event_wrapper(overlap, num_bins):
        try:
            data_subdivided, data = transform_trackml_data(event_id='21100', overlap_theta=overlap, overlap_phi=overlap, num_bins_theta=num_bins, num_bins_phi=num_bins)
    ...
    for i in range(len(overlaps)):
        ...
        for j in range(len(num_bins)):
            efficiency_matrix[i,j] = results[j][0]
            efficiency_std_matrix[i,j] = results[j][1]
            average_size_matrix[i,j] = results[j][2]
    ...
    # save image to file
    fig.savefig('efficiency_score.png')
    ...
    ax.set_title("Average number of hits in each bin")
    fig.tight_layout()
    # plt.show()

    # save image to file
    fig.savefig('average_number_hits.png')