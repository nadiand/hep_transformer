...
def load_trackml_data(data_path, normalize=False):
    ...
    def extract_track_params_data(event_rows):
        ...
        return processed_event_track_params_data
    ...
    print([torch.Tensor(hits) for hits in grouped_hits_data.values])
    # test = np.stack(grouped_hits_data.values)
    hits_data = torch.nested.nested_tensor([torch.Tensor(hits) for hits in grouped_hits_data.values])
