...
checkpoint = torch.load("trackml_1to50tracks_3out_last", map_location=torch.device('cpu'))
transformer.load_state_dict(checkpoint['model_state_dict'])
pytorch_total_params = sum(p.numel() for p in transformer.parameters() if p.requires_grad)
print("Total trainable params: {}".format(pytorch_total_params))

max_nr_hits = 1200
hits_data, track_params_data, track_classes_data = load_trackml_data(data_path="trackml_50to100tracks.csv", max_num_hits=max_nr_hits)
